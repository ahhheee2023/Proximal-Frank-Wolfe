...
% m = 800;   % the number of observations for each task; The observation matrix Phi_t is of size (m, N);
m_list = [500, 800];
for mm = 1:length(m_list)
    m = m_list(mm);
    nf = 0.05;    % noise factor on the observations; 0.1 and 0.05 is hard: rec_err is about 0.12 even m=800,1000
    Phi_all = cell(T,1);
    b_all = cell(T,1);
    L_all = zeros(T, 1);
    for t = 1:T
    ...
    filename = ['MTL-instance' '---' 'm-' int2str(m)  '--'  'nf-'  num2str(nf, '%2.0e')];
    ...
    mu_list = 0.01 + 0.01*(0:20);
    nnz_rec = zeros(1,1);
    rec_err_rec = zeros(1,1);
    for imu = 1:length(mu_list)
        ...
        rec_err_rec(imu) = rec_err;
        %     X = reshape(x, N, T);
        nnz_x = sum(abs(x)>1e-6);
        nnz_rec(imu) = nnz_x;
    end
    nnz_orig = sum(abs(xorig)>0);
    
    filename = ['MTL-cv' '-m-' int2str(m)  '-nf--'  num2str(nf, '%2.0e')  '-scalebeta' '--'  ...
        num2str(opts.scale_beta, '%2.0e') '-maxiter-' int2str(opts.maxiter)];
    save(filename, 'mu_list', 'nnz_rec' , 'rec_err_rec')
