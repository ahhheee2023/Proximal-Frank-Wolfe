function [] = exp_hc_l1_proj_nonegtvOrth_table(n,deriv_num,num_points,DDS_tol,ratio_list,save_to_file)
...
for k = 1:num_points
    ...
    obj_vals_DDS{k} = vecnorm(x_DDS{k}-u, 1);
end
...
%     opts.verbose_freq = 10;
    ...
    opts.stop.f = @(x) (norm(x-u, 1));
    opts.stop.obj_thresh = min(obj_vals_DDS{k})*(1+min(ratio_list));
    opts.stop.max_time = runtime_DDS(k); 
      
    [x_FW{k},y_FW{k},itr_FW(k),gap_FW{k},feas_FW{k},runtime_FW{k}] = proxFW_HP_exp(u, poly, e, c, opts);

    %calculate objective values
    obj_vals_FW{k} = vecnorm(x_FW{k} - u, 1);
    obj_vals_FW{k} = obj_vals_FW{k}';
end
...
for k = 1:num_points
    ...
    [x_FW_ele{k},y_FW_ele{k},itr_FW_ele(k),gap_FW_ele{k},feas_FW_ele{k},runtime_FW_ele{k}] = proxFW_HP_exp(d(:,k), poly, e, c, opts);

    %calculate objective values
    obj_vals_FW_ele{k} = vecnorm(x_FW_ele{k}-u, 1);
    obj_vals_FW_ele{k} = obj_vals_FW_ele{k}';
end
...
for i = 1:length(ratio_list)
    for k = 1:num_points
        if ~isempty(runtime_FW{k}( logical( ( (obj_vals_FW{k}-obj_vals_DDS{k})<=obj_vals_DDS{k}*ratio_list(i) ) .* feas_FW{k} ) ))
            t_FW(i,k) = min(runtime_FW{k}( logical( ( (obj_vals_FW{k}-obj_vals_DDS{k})<=obj_vals_DDS{k}*ratio_list(i) ) .* feas_FW{k}) ) );
            success_FW(i) = success_FW(i)+1;
        end
        if ~isempty(runtime_FW_ele{k}( logical( ( (obj_vals_FW_ele{k}-obj_vals_DDS{k})<=obj_vals_DDS{k}*ratio_list(i) ) .* feas_FW_ele{k}) ) )
            t_FW_ele(i,k) = min(runtime_FW_ele{k}( logical ( ( (obj_vals_FW_ele{k}-obj_vals_DDS{k})<=obj_vals_DDS{k}*ratio_list(i) ) .* feas_FW_ele{k} ) ) );
...
fprintf("relative error;\t \t relTime_FW;std;success;\t\t relTime_FW_ele;std;success\n")
for i = 1:length(ratio_list)
    fprintf("%.2e;\t%6.2f;%6.2f;%4.1f;\t%6.2f;%6.2f;%4.1f\n",ratio_list(i),...
...
if save_to_file
    ...
    fprintf(fileID,"error,FW,FWstd,FWsuccess,FWEl,FWElstd,FWElsuccess\n");
    for i = 1:length(ratio_list)
